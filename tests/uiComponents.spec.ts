...
test('tooltips', async({page}) => {
    ...
    //page.getByRole('tooltip') //only works if a role tooltip created
...
test('web tables', async({page}) => {
    ...
    const tRowById = page.getByRole('row', {name:'11'}).filter({has: page.locator('td').nth(1).getByText('11')})
    await tRowById.locator('.nb-edit').click()
    await page.locator('input-editor').getByPlaceholder('E-mail').clear()
    await page.locator('input-editor').getByPlaceholder('E-mail').fill('test@test.com')
    await page.locator('.nb-checkmark').click()
    await expect(tRowById.locator('td').nth(5)).toHaveText('test@test.com')
